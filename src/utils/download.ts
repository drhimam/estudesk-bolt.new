...
export function generateDeadlinesText(opts: ExportOptions): string {
  ...
  lines.push(`Semester:  ${semesterName || 'General'}`);
...
export function generateDeadlinesMarkdown(opts: ExportOptions): string {
  const { studentName, semesterName, deadlines, subjects, scopeLabel, grouping } = opts;
  const sorted = [...deadlines].sort((a, b) => a.dueDate - b.dueDate);
  const groups =
    grouping === 'subject'
  ...
  const lines: string[] = [];
  lines.push(`# 📅 Academic Deadline Schedule`);
  if (semesterName) lines.push(`**Semester**: ${semesterName}  `);
  if (studentName) lines.push(`**Student**: ${studentName}  `);
  lines.push(`**Scope**: ${scopeLabel} • **View**: ${grouping === 'subject' ? 'Subject-wise' : 'Date-wise'} • **Generated**: ${date}`);
  lines.push('');

  if (sorted.length === 0) {
    lines.push('_No deadlines found._');
  } else {
    for (const g of groups) {
      const title = g.sublabel ? `${g.label} (${g.sublabel})` : g.label;
      lines.push(`### ${title} (${g.items.length})`);
      lines.push('');
      for (const d of g.items) {
        const sub = getSubjectName(d, subjects);
        const dueStr = formatDate(d.dueDate);
        const rel = relativeDeadlineLabel(d);
        const check = d.completed ? '[x]' : '[ ]';
        const desc = d.description ? `  \n  > 📝 *${d.description}*` : '';
        lines.push(`- ${check} **${d.title}** — \`${dueStr}\` (${rel}) • *${sub}*${desc}`);
      }
      lines.push('');
    }
  }
  ...
  lines.push('---');
  lines.push('*Generated by [eStudesk](https://estudesk.com) • Academic Workspace*');
  return lines.join('\n');
}

export function downloadDeadlinesAsText(opts: ExportOptions): void {
  const { semesterName, scopeLabel } = opts;
  const content = generateDeadlinesText(opts);
...
export function generateDeadlinesPDFDoc(opts: ExportOptions): { doc: jsPDF; filename: string } {
  const { studentName, semesterName, deadlines, subjects, scopeLabel, grouping } = opts;
  const sorted = [...deadlines].sort((a, b) => a.dueDate - b.dueDate);
  const groups =
    grouping === 'subject'
  ...
  const scopeSlug = scopeLabel ? scopeLabel.replace(/\s+/g, '-').toLowerCase() : 'all';
  const stamp = getFileTimestamp();
  const filename = `deadlines-${semSlug}-${scopeSlug}_${stamp}.pdf`;

  return { doc, filename };
}

export function downloadDeadlinesAsPDF(opts: ExportOptions): void {
  const { doc, filename } = generateDeadlinesPDFDoc(opts);
  doc.save(filename);
}
